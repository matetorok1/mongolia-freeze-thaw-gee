 ...
 * Export Format: Multi-Band GeoTIFFs
 ***************************************************************/
...
// --- 5. DATA AGGREGATION (FOR CONSOLE CHART ONLY) ---
...
// --- 7. SPATIAL AGGREGATION & GeoTIFF EXPORT (DRIVE) ---
// Stacks all daily images for a winter into one multi-band image
function processYearImage(y) {
  var startDate = ee.Date.fromYMD(y, 10, 1);
  var endDate   = ee.Date.fromYMD(y + 1, 4, 30);
  var nDays     = endDate.difference(startDate, 'day').toInt();
  ...
  var dailyImages = ee.ImageCollection(dates.map(function(d){
    var img = classifyFreezeThaw(d);
    // Rename the band to the specific date (e.g., '2014_10_01')
    var dateStr = ee.Date(d).format('YYYY_MM_dd');
    return img.rename(dateStr);
  }));

  // Flatten the ImageCollection into a single multi-band image
  return dailyImages.toBands();
}

for (var y = startYear; y <= endYear; y++) {
  Export.image.toDrive({
    image: processYearImage(y),
    description: 'FreezeThaw_Mongolia_Winter_TIFF_' + y + '_' + (y + 1),
    folder: 'EarthEngine_Exports', 
    scale: 1000, // Native MODIS resolution is ~1km
    region: AOI,
